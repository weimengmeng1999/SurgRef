...
def get_video_extension(image_root, video_name, first_frame):
    """Check first frame to determine extension for entire video"""
    base_path = os.path.join(image_root, 'JPEGImages', video_name, first_frame)
...
def load_mevis_json(image_root, json_file):
    ...
    for vid_dict in tqdm(metas):
        record = {}
        ext = get_video_extension(image_root, vid_dict['video'], vid_dict["frames"][0])
        record["file_names"] = [
                    os.path.join(image_root, 'JPEGImages', vid_dict['video'], vid_dict["frames"][i] + ext)

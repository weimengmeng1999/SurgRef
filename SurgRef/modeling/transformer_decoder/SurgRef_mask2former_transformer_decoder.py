...
class SurgRefMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, clip_mask_features, lang_feat, lang_mask, mask = None):
        ...
        text_features = repeat(lang_feat, 'b c -> b t q c', t=bs, q=self.num_queries)
        b1, t, q, c = text_features.shape
        output = text_features.reshape(q, b1*t, c)
        frame_queries = []
        predictions_class = []
        predictions_mask = []
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size):
        ...
        # print(f"Original mask_embed shape: {mask_embed.shape}")  # (B, Q, C)
        # print(f"Original mask_features shape: {mask_features.shape}")  # (B', C, H, W)

...
class SpatialImageLanguageAttention(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        # print(f"l.shape before reshape: {l.shape}")
        ...
        # print(f"Expected shape: ({B}, {self.num_heads}, {self.key_channels // self.num_heads}, {n_l})")
        ...
        # # Get the correct key.shape calculation method
        # num_heads = self.num_heads
        # expected_key_shape = (B, num_heads, key.shape[1] // num_heads, n_l)

        # key = key.unsqueeze(1)
        # print(f"Fixing key shape: {key.shape} -> Expected: {expected_key_shape}")
        # # Fix reshape logic to ensure key_channels / num_heads does not cause shape mismatch
        # query = query.view(B, num_heads, HW, self.key_channels // num_heads).permute(0, 1, 2, 3)  
        # key = key.view(B, num_heads, self.key_channels // num_heads, n_l)  
        # value = value.view(B, num_heads, self.value_channels // num_heads, n_l)  

...
#SBATCH --output=mevis_J&F.out   # Standard output file
#SBATCH --error=mevis_J&F.err    # Error output file
...
#     OUTPUT_DIR outputs/train_output_endovis_rs17

python train_net_SurgRef.py \
    --config-file configs/SurgRef_SWIN_bs8.yaml \
    --num-gpus 4 --dist-url auto --eval-only \
    MODEL.WEIGHTS .../MeViS/outputs/train_output_endovis_rs17/model_final.pth \
    OUTPUT_DIR outputs/valu_output_endovis_rs17

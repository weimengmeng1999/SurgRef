...
def eval_queue(q, rank, out_dict, mevis_pred_path):
    while not q.empty():
        ...
        exp_dir = f"{mevis_pred_path}/{vid_name}/{exp}"
        if not os.path.exists(exp_dir) or not os.listdir(exp_dir):
            print(f"Warning: {exp_dir} is empty or does not exist!")
            out_dict[exp_name] = [0, 0]
            continue

        # Filter out frame files
        frame_files = sorted(
            [f for f in os.listdir(exp_dir) if f.endswith(".png")],
            key=lambda x: int(''.join(filter(str.isdigit, x)))  # Extract numbers for sorting
        )

        # Take the frame with the smallest number as pred_0
        if not frame_files:
            print(f"Warning: No valid PNG files found in {exp_dir}")
            out_dict[exp_name] = [0, 0]
            continue

        pred_0_path = os.path.join(exp_dir, frame_files[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mevis_pred_path", type=str, default="outputs/valu_output_endovis_rs17/inference")
    parser.add_argument("--save_name", type=str, default="outputs/endovis_rs17_valu_mevis_test.json")
